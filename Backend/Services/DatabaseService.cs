...
{
    /// <summary>
    /// Centralised data-access layer for FundForest.
    /// All SQL lives here; ViewModels never touch raw connections.
    /// </summary>
    public class DatabaseService
    {
        // ── Connection string ─────────────────────────────────────────────────
        // Edit these values (or read from app.config / environment) before shipping.
// Make sure the password matches your actual MySQL root password
private readonly string _connectionString =
    "Server=localhost;Port=3306;Database=fundforest;" +
    "Uid=root;Pwd=your_password;CharSet=utf8mb4;";
        ...
        public Admin? ValidateLogin(string username, string password)
        {
            ...
            using var cmd  = new MySqlCommand(
                "SELECT AdminID, Username, Password, FullName, Role FROM Admins WHERE Username=@u LIMIT 1", conn);
            ...
            // Accept plain-text comparison for dev, prefer BCrypt in production:
            // bool valid = BCrypt.Net.BCrypt.Verify(password, storedHash);
            // Plain text comparison to test
        ...
        {
            using var conn = GetConnection();
            string sql = "INSERT INTO Donors (DonorName,DonationType,ContactInfo,Address) VALUES(@n,@t,@c,@a)";
            using var cmd = new MySqlCommand(sql, conn);
        ...
        {
            using var conn = GetConnection();
            string sql = "UPDATE Donors SET DonorName=@n,DonationType=@t,ContactInfo=@c,Address=@a WHERE DonorID=@id";
            using var cmd = new MySqlCommand(sql, conn);
            BindDonor(cmd, d);
        ...
        {
            using var conn = GetConnection();
            string sql = @"INSERT INTO Donations (DonorID,Amount,DonationType,GoodsDescription,DonationDate,Notes)
                           VALUES(@did,@amt,@t,@gd,@date,@n)";
            using var cmd = new MySqlCommand(sql, conn);
        ...
        {
            using var conn = GetConnection();
            string sql = @"UPDATE Donations SET DonorID=@did,Amount=@amt,DonationType=@t,
                           GoodsDescription=@gd,DonationDate=@date,Notes=@n WHERE DonationID=@id";
            using var cmd = new MySqlCommand(sql, conn);
            BindDonation(cmd, d);
        ...
        public List<Models.Program> GetPrograms(string search = "")
        {
            ...
            using var conn = GetConnection();
            string sql = "SELECT * FROM Programs WHERE ProgramName LIKE @s ORDER BY ProgramID DESC";
            using var cmd = new MySqlCommand(sql, conn);
            cmd.Parameters.AddWithValue("@s", $"%{search}%");
            using var rd = cmd.ExecuteReader();
        ...
        {
            using var conn = GetConnection();
            string sql = @"INSERT INTO Programs (ProgramName,StartDate,EndDate,TargetAudience,Barangay,Description,Status)
                           VALUES(@n,@sd,@ed,@ta,@br,@desc,@st)";
            using var cmd = new MySqlCommand(sql, conn);
        ...
        {
            using var conn = GetConnection();
            string sql = @"UPDATE Programs SET ProgramName=@n,StartDate=@sd,EndDate=@ed,
                           TargetAudience=@ta,Barangay=@br,Description=@desc,Status=@st WHERE ProgramID=@id";
            using var cmd = new MySqlCommand(sql, conn);
            BindProgram(cmd, p);
        ...
        {
            using var conn = GetConnection();
            string sql = @"UPDATE Distributions SET BeneficiaryID=@bid,ProgramID=@pid,DistributionDate=@date,
